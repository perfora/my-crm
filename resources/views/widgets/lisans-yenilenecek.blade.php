...
@php
    try {
        ...
        $islenmisKayitIds = collect([]);

        if (\Illuminate\Support\Facades\Schema::hasTable('lisans_yenileme_kayitlari')) {
            $islenmisKayitIds = \Illuminate\Support\Facades\DB::table('lisans_yenileme_kayitlari')
                ->pluck('source_is_id');
        }
    ...
    $yenilenecekler = \App\Models\TumIsler::where('tipi', 'Kazanıldı')
        ...
        ->when($islenmisKayitIds->isNotEmpty(), function ($query) use ($islenmisKayitIds) {
            $query->whereNotIn('id', $islenmisKayitIds);
        })
        ->with(['musteri', 'marka'])
    ...
    // Görünenler henüz işlenmemiş kayıtlar
    $acilmamis = $yenilenecekler->count();
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($yenilenecekler as $is)
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-center whitespace-nowrap">
                            ...
                            <button
                                type="button"
                                onclick="yenilemeIsaretle({{ $is->id }}, this)"
                                class="inline-flex items-center justify-center w-8 h-8 rounded bg-green-600 hover:bg-green-700 text-white transition ml-1"
                                title="Açıldı olarak işaretle">
                                ✓
                            </button>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-gray-500 js-empty-row">
...
<script>
function removeLisansRow(button) {
    ...
    if (tbody && tbody.querySelectorAll('tr').length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="px-4 py-8 text-center text-gray-500 js-empty-row">🎉 Harika! Tüm lisanslar için yenileme kaydı açılmış.</td></tr>';
    }
}

function yenilemeAc(isId, button) {
...
function yenilemeIsaretle(isId, button) {
    button.disabled = true;
    button.innerHTML = '...';
